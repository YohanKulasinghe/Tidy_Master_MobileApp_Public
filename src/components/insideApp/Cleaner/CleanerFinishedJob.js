...
import { TouchableOpacity, AsyncStorage, ActivityIndicator, StyleSheet,Image } from 'react-native';
...
class CleanerFinishedJob extends Component {
...
getCompletedJobs(){
  ...
  fetch(host.config.hostname+'/cleaner/getdonejobs', {
    method: 'POST',
    headers: {
      ...
      <Container>
        <Content>
          <List dataArray={this.state.data} renderRow={(item) =>
            ...
            <ListItem >
              <Body>
              <TouchableOpacity onPress={() => {Actions.CleanerFinishedJobDetails({postid:item.postid})}}>
...
export default CleanerFinishedJob;